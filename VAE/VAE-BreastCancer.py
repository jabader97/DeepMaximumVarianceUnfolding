...
class VAE(nn.Module):
    ...
    def forward(self, x: Variable) -> (Variable, Variable, Variable):
        latent, mu, logvar = self.encode(x.view(-1, 30))
...
def loss_function(recon_x, x, mu, logvar) -> Variable:
    ...
    BCE = F.binary_cross_entropy(recon_x, x.view(-1, 30))
...
def original_clean():
    ...
    for i in range(len(labels)):
        if labels[i] == 'B':
            ...
            temp_labels[i] = 1
    # normalize
    temp_data = torch.from_numpy(normalize(dataset)).float()
